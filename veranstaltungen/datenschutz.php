<?php
/**
 * Datenschutzerklärung von was.läuft – die eigene, kurze Fassung.
 *
...
$text = trim(wl_text('recht_datenschutz'));
if ($text === '') {
    $aus = trim(wl_setting('privacy_url', ''));
    if ($aus !== '') { header('Location: ' . $aus); exit; }
    http_response_code(404);
    wl_head(wl_t('f_datenschutz'), '', '', true);
    wl_nav();
    echo '<main class="wl-wrap wl-recht"><h1>' . h(wl_t('f_datenschutz')) . '</h1>'
